...
/**
 The Plurality definition defines which version of the "`<v1:v2:v3>`" plural strings to use depending on the PluralValue it is given.
 
 Here are the rules that are used:
 - Each possible version should be represented by a “zone;”
 - A zone is as follow: “`(zone_content)↓??`” where “zone_content” is defined later and the number of “`?`” and “`↓`” ∈ ℕ (`↓`s must be before `?`s);
 ...
 - In a zone, there is a list of values, separated by a colon. Values can be:
    - A single number (int or float, no leading + or 0 allowed).
      If the number is an int, only ints will match, otherwise ints and floats can match;
    ...
    - An continuous interval (can match ints and floats), with this syntax:
      `[n→m]`, where:
       ...
       - the opening and closing separator regions can be either opening or closing square brackets (to include or exclude the value)
    - A custom “globbing” expression:
       - Must start with “`^`”, and end with “`$`”;
       - Match is done on given plural value’s `fullStringValue`;
       - By default, the glob will match positive and negative numbers (the sign is ignored).
         To force positive or negative matching, you must add a “`+`” or “`-`” resp. after the “`^`”;
       ...
       - Range (same semantics as standard regex’s `[]` where `-` is replaced by `→`.
         “`→`” is specifically reserved for a “digit to digit” use.
         The decimal separator is NOT allowed in brackets.):
       ...
       - No other characters than the one with meaning in regard to the above rules are allowed in a globbing expression.
         To summarize:
       ...
       - Example: “`^*3{.*}$`” matches all floats or int whose int value ends with 3
       - Example: “`^{*.}*3$`” matches any number (int or float) ending with 3 (13, 1.53, etc.)
 ...
 If the number of zones does not match the number of versions, the following algorithm will be used in order to match a version:
    - If the number of version is greater than the number of zones a warning will be printed in the logs.
      The first matching zone will be used.
    - If the number of versions is smaller than the number of zones,
      some zones will be removed until the number of zones equal the number of versions,
      using the following algorithm:
       - The zones with the more question marks are removed first;
       - If two zones have the same number of question marks, the **LAST** one is removed first.
       - A warning is printed in the logs if a non-question-marked zone have to be removed.
 
 If more than one zone match, the first that matches and has the lowest number of `↓` is used.
 If no zone matches, the **LAST** version will be used and an info message will be logged.
 ...
  **Examples**:
    ...
    - Custom plural for 1, then 2, 5, 6.3 and anything (including floats) between 6.4 (excluded) and 8 (included), then ints from 6 to 9:
      ```text
      “(1)(2:5:6.3:]6.4→8])(6→9)”
      ```
      - Note: As there is no `(*)` zone, the latest one (6 to 9) will match for non-matching numbers.
 
        With `<zone1:zone2:zone3>`:
         - For 1, the value will be zone1
         - For 2, the value will be zone2
         - For 6.3, the value will be zone2
         - For 6.4, the value will be zone3 (6.4 does not match any zone, so it matches the latest)
         - For 6.31, the value will be zone3 (6.31 does not match any zone, so it matches the latest)
         - For 6.41, the value will be zone2
         - For 8, the value will be zone2 (both zone2 and zone3 matches, but the first one is used)
 
    - Custom plural for 1, 2, 3 then 2 to 5 (floats too) then any int below 7 then the rest:
      ```text
      “(1→3)([2→5[)(→6)(*)”
      ```
 
    - Custom plural for 1, 2, 3 then 2.5 to 5 (floats too) then any int above 5 with star zones at the beginning:
      ```text
      “(*)↓↓(*.)↓(1→3)([2.5→5])(6→)”
      ```
...
#endif
				Conf.logger?.warning("Got malformed plurality definition string “\(string)”. Attempting to continue anyway...")
				continue
			}
...
#if canImport(os)
				if #available(macOS 10.12, tvOS 10.0, iOS 10.0, watchOS 3.0, *) {
					Conf.oslog.flatMap{ os_log("Got zone str (%@), which I cannot parse into a zone", log: $0, type: .info, curZoneStrMinusOpeningParenthesis) }}
#endif
				Conf.logger?.warning("Got zone str (\(curZoneStrMinusOpeningParenthesis)), which I cannot parse into a zone")
			}
...
#if canImport(os)
			if #available(macOS 10.12, tvOS 10.0, iOS 10.0, watchOS 3.0, *) {
				Conf.oslog.flatMap{ os_log("No zones matched for given predicate in plurality definition %{public}@. Returning latest version.", log: $0, type: .info, String(describing: self)) }}
#endif
			Conf.logger?.warning("No zones matched for given predicate in plurality definition \(String(describing: self)). Returning latest version.")
...
#if canImport(os)
			if #available(macOS 10.12, tvOS 10.0, iOS 10.0, watchOS 3.0, *) {
				Conf.oslog.flatMap{ os_log("Had to remove at least one non-optional zone in plurality definition %@ in order to get version idx for %d version(s).", log: $0, type: .info, String(describing: self), numberOfVersions) }}
#endif
			Conf.logger?.warning("Had to remove at least one non-optional zone in plurality definition \(String(describing: self)) in order to get version idx for \(numberOfVersions) version(s).")
		}
